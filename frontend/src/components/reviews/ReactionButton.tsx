...
export default function ReactionButton({
  ...
  useEffect(() => {
    async function load() {
      try {
        const res = await getReactionSummary(reviewId);
        setMyReaction(res.data.myReaction);
        setTotal(res.data.total);
      } catch {
        // লগইন না থাকলেও summary আসার কথা (OptionalJwtAuthGuard) — ব্যর্থ হলে চুপচাপ ignore
      }
    }
    load();
  }, [reviewId]);
  ...
  async function applyReaction(type: ReactionType) {
    ...
    try {
      if (previous === type) {
        // একই reaction-এ আবার ক্লিক করলে remove হয়ে যাবে
        await unreactToReview(reviewId);
        setMyReaction(null);
      ...
      // silent — পরে আবার চেষ্টা করা যাবে
    } finally {
      setLoading(false);
    }
  }
